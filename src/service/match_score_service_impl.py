...
class MatchScoreServiceImpl(MatchScoreService):
    ...
    def play_match(self, uuid: str, point: int = None) -> MatchDTO:
        ...
        if point is None:
            return MatchDTO(uuid, scoreboard.to_dict())

        if match.winner is not None:
            winner_name = match.winner_rel.NAME
            return MatchDTO(uuid, scoreboard.to_dict(), winner_name)
        ...
        self.__match.update_match(MatchDTO(uuid, score_json))
    ...
    def __finalize_match_with_winner(self, uuid: str, scoreboard: ScoreboardTennis) -> MatchDTO:
        winner_name = scoreboard.winner_player.name
        winner = self.__player.get_player(winner_name)
        score_json = json.dumps(scoreboard.to_dict())
        self.__match.update_match(MatchDTO(uuid, score_json, winner.ID))
        return MatchDTO(uuid, scoreboard.to_dict(), winner.NAME)
